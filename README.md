...
```text
MONGODB_URI=
GROQ_PLANNER_API_KEY_1=
GROQ_PLANNER_API_KEY_2=
...
Set `GROQ_PLANNER_API_KEY_1`, `GROQ_PLANNER_API_KEY_2`, and `GROQ_PLANNER_API_KEY_3`
to give the three-stage planner one key per stage. Charon tries the next planner key on
429/503 or auth failure without logging secret values. Set `GROQ_RESPONSE_API_KEY`
for the response writer; Charon uses exactly one response key.
...
Charon preflights the whole sequence, executes steps in order, and can pass nested results such as an earlier Meet link, public id, or listed item into later steps. The planner prompt includes the full action catalog, context priority, reference rules, and finite-step workflow rules so natural requests such as "list active meetings", "move the last meeting to Friday 6pm CST", simple schedules, and relative reminders are planned by the three-stage planner. After local tools run, action/list/update/schedule/reminder replies use the deterministic response writer so unrelated chat context cannot leak into operational confirmations. Longer sequences also use the deterministic response writer, avoiding another model call. Command mode uses no LLM calls.
